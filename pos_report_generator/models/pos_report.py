    ...
    def _get_report_sub_lines(self, data, report, date_from, date_to):
        ...
        if data.get('report_type') == 'report_by_order':
            ...
            if  not data.get('date_from') and  not data.get('date_to'):
                query += term 
            query += " order by l.date_orders desc group by l.user_ids,res_users.partner_id,res_partner.name,l.partner_ids,l.date_orders,pos_session.name,l.session_ids,l.name,l.price_subtotal,l.id"
