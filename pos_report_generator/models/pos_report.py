...
class PosReportGenerator(models.Model):
    ...
    def pos_report(self, option):
        ...
        if user.has_group('mobile_shop.group_Store_manager'):
            is_managed = True
        else:
            is_managed = False
    ...
    def _get_report_sub_lines(self, data, report, date_from, date_to):
        ...
        if data.get('report_type') == 'report_by_order':
            query = '''
                   select 
                    l.date_orders as date_order,
                    l.partner_ids as partner_id,
                    l.price_subtotal as amount_total,
                    l.user_ids as user_id,
                    res_partner.name,
                    l.name as shop,
            ...
            if  not data.get('date_from') and  not data.get('date_to'):
                query += term 
            query += " group by l.user_ids,res_users.partner_id,res_partner.name,l.partner_ids,l.date_orders,pos_session.name,l.session_ids,l.name,l.price_subtotal,l.id order by l.date_orders desc"
