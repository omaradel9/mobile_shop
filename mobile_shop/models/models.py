...
class productTemplate(models.Model):
    ...
    def get_product_label(self):
        ...
        return self.product_id.product_tmpl_id.process()
...
class productTemplate(models.Model):
    ...
    vendor_name = fields.Many2one(
        string="المورد",
        comodel_name="res.partner",domain="[('supplier_rank', '>',0),('repair_vendor', '=', False)]",
        context="{'default_repair_vendor':'False}")  
    
    @api.model
    def _default_image(self):
        with open(get_module_resource('mobile_shop', 'static/src/img', 'gadgets.png'), 'rb') as f:
            return base64.b64encode(f.read())
    
    image_1920  = fields.Binary(default=_default_image, attachment=True,)
    @api.model
    ...
    def _prepare_report_data(self):
        ...
        if not self.barcode:
            raise UserError(_('You need to set a barcode.'))
        ...
        data = {
            ...
            'layout_wizard': 104,
            'quantity_by_products':  self.qty_available if self.qty_available else 1,
        }
        return xml_id, data
